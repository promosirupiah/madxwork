...
return false;};}));var alltitle=$('title').html();var reid=/(\[id\:)(.*?)(\])/gi;var reeid=/(\{id\:)(.*?)(\})/gi;var reen=/(\[en\:)(.*?)(\])/gi;var reeen=/(\{en\:)(.*?)(\})/gi;function langid(time){$('.en').hide(time);$('.id').show(time);$.cookie("lang",'id',{expires:1});$('a#id').css({'color':'red',});$('a#en').css({'color':'white',});$('title').html(alltitle.replace(reid,'$2').replace(reen,''));$('a').each(function(){try{var title=$(this).attr('orititle').replace(reeid,'$2').replace(reeen,'')}catch(error){var title=$(this).attr('orititle')}
...
function langen(time){$('.id').hide(time);$('.en').show(time);$.cookie("lang",'en',{expires:1});$('a#en').css({'color':'red',});$('a#id').css({'color':'white',});$('title').html(alltitle.replace(reid,'').replace(reen,'$2'));$('a').each(function(){try{var title=$(this).attr('orititle').replace(reeen,'$2').replace(reeid,'')}catch(error){var title=$(this).attr('orititle')}
$(this).attr({'title':title,})})
